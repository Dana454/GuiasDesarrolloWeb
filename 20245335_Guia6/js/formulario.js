...
const addPaciente = function () {
    ...
    if (window.indiceEdicion !== undefined) {
        arrayPaciente[window.indiceEdicion] = [
            nombre,
            apellido,
            fechaNacimiento,
            sexo,
        ...
        window.indiceEdicion = undefined;
        limpiarForm();
        imprimirPacientes();
        return;
    }
...
function imprimirFilas() {
    ...
    arrayPaciente.forEach((element) => {
        $fila += `<tr>
            ...
            <td>${element[5]}</td>
            <td>
                <button onclick="editarPaciente(${contador - 1})" type="button" class="btn btn-primary" >
                    <i class="bi bi-pencil-square"></i>
                </button>
                <button onclick="eliminarPaciente(${contador - 1})" type="button" class="btn btn-danger">
...
function editarPaciente(index) {
    ...
    // Buscar el option por texto
    let opciones = Array.from(cmbPais.options);
    let opcionEncontrada = opciones.find(opt => opt.text === paciente[4]);
    cmbPais.value = opcionEncontrada ? opcionEncontrada.value : 0;
    ...
    mensaje.innerHTML = "Editando paciente. Modifica los datos y presiona Agregar.";
    toast.show();
}
